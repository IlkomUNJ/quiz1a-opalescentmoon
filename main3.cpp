...
enum AdminPrompt{ACCOUNT_MANAGEMENT, SYSTEM_REPORT, GO_BACK};
enum ManagementPrompt{VIEW_USERS, VIEW_USER_DETAILS, SEEK_USER, CREATE_ACCOUNT, REMOVE_USER};
enum SystemPrompt{TOTAL_USER, TOTAL_BANK};
...
int main() {
    ...
    AdminPrompt admPrompt = ACCOUNT_MANAGEMENT;
        ...
        switch (prompt) {
                ...
                while (regPrompt != BACK){
                    ...
                    switch (regPrompt) {
                        case CREATE_BUYER:
                        {
                            ...
                            bankCustomer account(bankID, bankName, balance);
                            Buyer newBuyer(buyerIDs++, buyerName, account);

                            buyers.push_back(newBuyer);
                            cout << "Successfully created buyer account";
                        }
                        ...
                        { 
                            ...
                            bankCustomer account(bankID, bankName, balance);
                            Seller newSeller(sellerIDs++, sellerName, account);

                            sellers.push_back(newSeller);
                            cout << "Successfully created seller account";
                        }
            ...
            case ADMIN_LOGIN:
                ...
                while (admPrompt != GO_BACK)
                {
                    ...
                    int admChoice;
                    cin >> admChoice;
                    admPrompt = static_cast<AdminPrompt>(admChoice - 1);
                    switch (admPrompt)
                    {
                    case ACCOUNT_MANAGEMENT:
                        cout << "Selected Account Management" << endl;
                        break;
                    case SYSTEM_REPORT:
                        cout << "Selected System Report" << endl;
                        break;
                    case GO_BACK:
                        cout << "Back selected" << endl;
                        admPrompt = ACCOUNT_MANAGEMENT;
                        break;
                    default:
                        cout << "Invalid Option." << endl;
                        break;
